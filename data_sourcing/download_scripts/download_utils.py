...
and Supabase integration for tracking download progress.

Supabase integration is enabled by default but can be disabled by setting
SUPABASE_ENABLED = False in supabase_config.py
"""
...
def with_retry(func, args, column_info, session_id):
    ...
    """
    wait_times = [30, 60, 120, 180, 300]  # 30, 60 seconds
...
def download_and_process_youtube(youtube_url, output_filename):
    ...
    try:
        ...
        # Setup proper temp and final paths
        temp_path = os.path.join(temp_dir, f'temp_{output_filename}.%(ext)s')
        temp_opus = os.path.join(temp_dir, f'temp_{output_filename}.opus')
        final_opus = f'{output_filename}.opus'
...
def download_and_process_generic_video(url: str, output_filename: str) -> bool:
    ...
    try:
        ...
        # logging if we are using a proxy
        if download_command[2] == '--proxy':
...
def get_video_url(url: str, patterns: List[str]) -> Optional[str]:
    """
    Extract video URL from a webpage using a list of regex patterns.
    ...
    Returns:
        str: The extracted video URL if found, None otherwise
    """
    response = None
    try:
        ...
        for pattern in patterns:
            if matches := re.findall(pattern, response.text, re.IGNORECASE):
                for match in matches:
                    ...
                    if url_to_test.startswith('//'):
                        # Extract protocol from original URL
                        protocol = url.split('://')[0] if '://' in url else 'https'
                        url_to_test = f"{protocol}:{url_to_test}"
                    
                    # Add a small delay before the HEAD request
                    time.sleep(random.uniform(1, 2))
                    
                    try:
                        head_response = requests.head(url_to_test, timeout=5, proxies=PROXY_CONFIG)
                        if head_response.status_code == 200:
                            logging.info(f"Found valid video URL: {url_to_test}")
                            return url_to_test
                    except requests.exceptions.RequestException as e:
                        logging.warning(f"Failed to validate URL {url_to_test}: {str(e)}")
                    
                    # Add a small delay between pattern attempts if the HEAD request fails
                    time.sleep(random.uniform(0.5, 1))
        
        # If we get here, no pattern matched
        logging.error(f"Could not find video URL in page: {url}")
        if response:
            logging.debug(f"Page source excerpt: {response.text[:1000]}...")
        return None
...
def download_and_process_generic_m3u8_link(url: str, output_filename: str) -> bool:
    ...
    This function is designed to handle various ways m3u8 streams are embedded in webpages.
    
    Args:
        url: URL of the webpage containing the video
        output_filename: Desired output filename (without extension)
    """
    # Patterns specific to m3u8 link extraction
    patterns = [
        # Original patterns for m3u8 streams
        r'(https?://[^\s<>"]+?/vod/_definst_/(?:mpflv):[^\s<>"]+?\.(?:mp4|m3u8))',
        r'(https?://[^\s<>"]+?/(?:playlist|manifest)\.(?:m3u8|mpd))',
        r'(https?://[^\s<>"]+?\.(?:m3u8|mpd)(?:\?[^\s<>"]*)?)',
        
        # JavaScript embedded m3u8 links
        r'file:[\s]*[\'"]([^\'"]+\.m3u8(?:\?[^\'"]*)?)[\'"]',
        r'file:[\s]*[\'"]((?:https?:)?//[^\s<>"\']*/[\w/:.-]+\.m3u8(?:\?[^\'"]*)?)[\'"]',
        r'file:[\s]*[\'"]((?:https?:)?//[^\s<>"\']*/(?:mp4|stream):[\w/:.-]+\.mp4/playlist\.m3u8(?:\?[^\'"]*)?)[\'"]',
        
        # Stream URL patterns
        r'streamUrl[\s]*[=:][\s]*[\'"]([^\'"]+)[\'"]',
        r'videoUrl[\s]*[=:][\s]*[\'"]([^\'"]+)[\'"]',
        
        # General m3u8 patterns
        r'[\'"]((?:https?:)?//[^\s<>"\']+?\.m3u8(?:\?[^\'"]*)?)[\'"]',
    ...
    try:
        # Extract the m3u8 URL using get_video_url with our specific patterns
        ...
        logging.error(f"Error processing m3u8 from {url}: {str(e)}")
        return False
