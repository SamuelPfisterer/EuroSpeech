    ...
    def extract_links_old_format(self, table) -> List[Dict]:
        ...
        for row in table.query_selector_all("tr"):
            ...
            for cell in cells[1:]:
                ...
                sitting_number = re.search(r'Session(?:e)? (?:no\. )?(\d+)', 
                                        html_link.inner_text(), 
                                        re.IGNORECASE)
                sitting_number = sitting_number.group(1) if sitting_number else None
...
def main():
    ...
    output_dir = "transcript_links"
    
    for legislation in scraper.legislation_periods.keys():
