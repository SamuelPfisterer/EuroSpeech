...
if __name__ == "__main__":
    ...
    transcript_csv_path = "scraping-parliaments-internally/lithuania/transcript_urls.csv"
    video_csv_path = "scraping-parliaments-internally/lithuania/video_links.csv"
    output_csv_path = "scraping-parliaments-internally/lithuania/lithuania_urls.csv"
    join_key_transcript = "date"
    join_key_video = "date"

    def extract_date_from_id(id_str: str) -> str:
        # Extract the last 8 characters which represent the date
        if id_str and len(id_str) >= 8:
            return id_str[-8:]
        return None
