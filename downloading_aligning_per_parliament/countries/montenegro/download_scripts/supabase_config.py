"""
Supabase configuration and utility functions for the Icelandic Parliament download system.
"""
...
# Flag to enable/disable Supabase (enabled by default)
SUPABASE_ENABLED = False  # Set to False to disable Supabase integration
...
class SupabaseClient:
    ...
    def __init__(self):
        if self._client is None and SUPABASE_ENABLED:
...
def get_supabase() -> Optional[Client]:
    ...
    return SupabaseClient().client if SUPABASE_ENABLED else None
...
def get_download_status(session_id: str) -> Optional[Dict[str, Any]]:
...
def get_parliament_progress() -> Optional[Dict[str, Any]]:
    """Get the current progress for the Icelandic parliament."""
    if not SUPABASE_ENABLED:
        return None
